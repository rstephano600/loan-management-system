...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
        <h2 class="h3 mb-0 text-dark">
            <i class="bi bi-cash-stack text-primary me-2"></i> New Client Loan Requests
        </h2>
        <a href="{{ route('loan_request_new_client.create') }}" class="btn btn-primary shadow-sm d-flex align-items-center fw-semibold">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <span class="badge bg-{{ 
                                        ...
                                        ($loan->status === 'rejected' ? 'danger-subtle text-danger' : 'secondary'))) 
                                    }} fw-semibold">
                                        {{ ucfirst($loan->status) }}
                                    </span>
                                </td>
                                ...
                                <td class="text-center text-nowrap">
                                    <div class="btn-group btn-group-sm" role="group">
                                        {{-- View --}}
                                        <a href="{{ route('loan_request_new_client.show', $loan->id) }}" 
                                           class="btn btn-outline-primary" data-bs-toggle="tooltip" title="View Details">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        {{-- Edit --}}
                                        <!-- <a href="{{ route('loan_request_new_client.edit', $loan->id) }}" 
                                           class="btn btn-outline-warning" data-bs-toggle="tooltip" title="Edit Loan">
                                            <i class="bi bi-pencil-square"></i>
                                        </a> -->
                                        {{-- Delete --}}
                                        <!-- <form action="{{ route('loan_request_new_client.destroy', $loan->id) }}" 
...
<script>
// Optional: Add some JavaScript to enhance the user experience
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Clear date inputs when the exact disbursed date is used and vice versa
    const disbursedDate = document.getElementById('disbursed_date'); // Note: 'Exact Disbursed Date' input was removed in the filter revamp for simplicity, but keeping this logic in case it's added back.
    ...
    // Logic to clear date inputs (kept for completeness, though 'disbursed_date' is currently not in the new layout)
    if (disbursedDate) {
        disbursedDate.addEventListener('change', function() {
            if (this.value) {
                disbursedFrom.value = '';
                disbursedTo.value = '';
            }
    ...
    if (disbursedFrom || disbursedTo) {
        [disbursedFrom, disbursedTo].forEach(input => {
            if (input) {
                input.addEventListener('change', function() {
                    if (disbursedDate && (disbursedFrom.value || disbursedTo.value)) {
                        disbursedDate.value = '';
                    }
                });
            }
