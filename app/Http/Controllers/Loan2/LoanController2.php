...
{
    /**
     * Display a listing of the loans with search and filters.
     */
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'repayment_frequency' => ['required', Rule::in(['daily', 'weekly', 'bi_weekly', 'monthly', 'quarterly'])],
            
            'disbursement_date' => 'required|date',
        ]);
        
        // 1. Initial Loan Creation Data
        ...
        // 2. Calculate schedule totals (using helper methods)
        ...
        $loanData = array_merge($validated, [
            ...
            'total_outstanding' => round($totalPayable, 2), // Total outstanding includes interest and principal initially
            'created_by' => Auth::id(),
            // The Loan model is missing installment_amount and first_payment_date fields, 
            // but these would typically be calculated and stored here or on the first schedule item.
        ]);

        $loan = Loan::create($loanData);

        // 4. Generate and Store Repayment Schedule (CRITICAL NEW STEP)
        try {
            $firstDueDate = $this->generateRepaymentSchedule($loan, $principal, round($installment, 2), $i, $n, $disbursementDate, $frequency);

            // 5. Update Loan with First Payment Date
            $loan->update([
            ...
            $loan->delete();
            return back()->withInput()->withErrors(['schedule_error' => 'Failed to generate repayment schedule: ' . $e->getMessage()]);
        }
    ...
    {
        // Validation for update is usually stricter or changes depending on the loan status.
        $validated = $request->validate([
            'client_id' => 'required|exists:clients,id',
            'loan_category_id' => 'required|exists:loan_categories,id',
            'group_center_id' => 'nullable|exists:group_centers,id',
            'principal_amount' => 'required|numeric|min:1', // If still editable
            'status' => ['required', Rule::in(['pending', 'approved', 'active', 'completed', 'defaulted', 'closed'])],
            'disbursement_date' => 'nullable|date',
            // ... include other editable fields
        ]);
        
        // Note: Changing financial fields like principal_amount or interest_rate on an active loan 
        // requires recalculating all amortization schedules, which is complex and omitted here.

        $validated['updated_by'] = Auth::id();

        // Handle closure date if status changes to completed/closed
        if (in_array($validated['status'], ['completed', 'closed']) && is_null($loan->closed_at)) {
    ...
    /**
     * Remove the specified loan from storage. (Usually restricted in production)
     */
    public function destroy(Loan $loan)
    {
        // Typically only allowed if the loan is in 'pending' status.
        if ($loan->status !== 'pending') {
            return back()->with('error', 'Cannot delete a loan that has been processed.');
        }
